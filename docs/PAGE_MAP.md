...
| `/admin/businesses/:businessId/inventory-input` | `AdminInventoryDetailPage` | `DashboardShell`, inventory readout | Yes | `admin` | `GET /admin/businesses/:id`, `GET /admin/businesses/:id/inventory-submissions/latest`, `GET /admin/users?limit=100&offset=0` | Detail input inventarisasi user untuk business tertentu dari monitoring admin, dengan tombol ke Sub Skor dan Laporan AI. Kartu ringkasan sudah dihapus dari halaman ini — dipindah ke Business Snapshot di `/sub-scores`. |
