...
| `/settings` | `SettingsPage` | `DashboardShell`, form profil, `GoogleLoginButton`, form setup/ubah password, `ThemeCustomizer` | Yes | `admin`, `user` | `PATCH /me`, `POST /me/google/link`, `POST /me/password/setup`, `PATCH /me/password` | Pengaturan profile nama dengan dialog konfirmasi, metode login readonly, tautkan Google, keamanan akun untuk setup password akun Google-only, ubah password mandiri, dan pengaturan tema lokal warna/dark mode. |
