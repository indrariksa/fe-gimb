...
| `/businesses/:businessId/sub-scores` | `SubScoresPage` | `DashboardShell`, subscore cards, radar SVG, bar chart, inventory insight charts, export buttons | Yes | `admin`, `user` | User: `GET /businesses/:id`, `GET /businesses/:id/inventory-submissions/latest`; Admin: `GET /admin/businesses/:id`, `GET /admin/businesses/:id/inventory-submissions/latest` | Detail enam sub-skor bisnis, insight operasional dari data inventarisasi, legend interpretasi skor, PDF report detailed analysis, dan XLSX multi-sheet. |
