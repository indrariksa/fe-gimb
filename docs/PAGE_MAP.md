...
| `/settings` | `SettingsPage` | `DashboardShell`, form profil, form setup/ubah password, `ThemeCustomizer` | Yes | `admin`, `user` | `PATCH /me`, `POST /me/password/setup`, `PATCH /me/password` | Pengaturan profile nama dengan dialog konfirmasi, email/role/metode login readonly, keamanan akun untuk setup password akun Google-only, ubah password mandiri, dan pengaturan tema lokal warna/dark mode. |
