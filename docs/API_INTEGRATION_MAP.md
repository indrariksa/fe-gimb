...
| Manual verify user email | `src/services/api/admin.ts` | PATCH | `/admin/users/{id}/email/verify` | Tidak ada | `User` | `AdminPage` menampilkan badge status email dan tombol `Verifikasi` untuk user yang belum verified, lalu confirmation dialog sebelum bypass manual. |
