...
| Get health report | `src/services/api/businesses.ts` | GET | `/businesses/{publicId}/health-report` | Path `publicId` | `AIReport` (`status`, `model`, `report`, `error_message`) | `AIReportPage` menampilkan state `processing`/`ready`/`failed`; `404` diperlakukan sebagai "belum eligible" (omzet di bawah threshold), bukan error. |
| Regenerate health report | `src/services/api/businesses.ts` | POST | `/businesses/{publicId}/health-report/regenerate` | Tidak ada | `AIReport` | `AIReportPage` memanggil ini dari tombol "Generate Ulang" saat status `failed`; backend menolak `409` di luar kondisi `failed`/`processing` macet. |

## Admin

| Fitur | File Service | Method | Endpoint | Request | Response | Penanganan Error |
| --- | --- | --- | --- | --- | --- | --- |
| Admin summary | `src/services/api/admin.ts` | GET | `/admin/dashboard/summary` | Tidak ada | `AdminSummary` | `AdminSummaryPage` menampilkan `Gagal memuat ringkasan admin` atau message error. |
| Admin analytics | `src/services/api/admin.ts` | GET | `/admin/dashboard/analytics` | Tidak ada | `AdminAnalyticsResponse` (`{ charts: AIReportChartData[] }`) | `AdminSummaryPage` merender 5 chart dari response (`businesses_by_industry`: bar, `status_distribution`: doughnut, `submissions_trend`: line, `user_growth`: line, `revenue_by_industry`: bar unit `"Rp"`) lewat `AdminAnalyticsChart`, ditambah 1 chart pie `user_status` yang dihitung di frontend dari `AdminSummary`. Error digabung dalam loading Ringkasan Admin. |
| Audit logs | `src/services/api/admin.ts` | GET | `/admin/audit-logs?limit={limit}&offset={offset}&level={level}&search={search}` | Query `limit`, `offset`, optional `level`, optional `search` | `ListResponse<AuditLog>` | Search/filter diproses server-side agar pagination mengikuti hasil filter; `AdminAuditLogPage` mengosongkan list/meta dan menampilkan `Gagal memuat audit log` atau message error. |
...
| Update business limit | `src/services/api/admin.ts` | PATCH | `/admin/settings/business-limit` | `{ value }` | `BusinessLimitSetting` | `AdminLimitPage` menampilkan setting message sukses/gagal. |
...
| Update user status | `src/services/api/admin.ts` | PATCH | `/admin/users/{id}/status` | `{ status }` | `User` | `AdminUsersPage.updateStatus` tidak membungkus try/catch lokal; error akan reject dari event handler. |
| Manual verify user email | `src/services/api/admin.ts` | PATCH | `/admin/users/{id}/email/verify` | Tidak ada | `User` | `AdminUsersPage` menampilkan badge status email dan tombol `Verifikasi` untuk user yang belum verified, lalu confirmation dialog sebelum bypass manual. |
...
| Diagnosis watchlist | `src/services/api/admin.ts` | GET | `/admin/diagnosis-watchlist?limit={limit}&offset={offset}` | Query `limit`, `offset` | `ListResponse<InventorySubmission>` | `AdminDiagnosisPage` mengosongkan list/meta dan menampilkan `Gagal memuat monitoring diagnosis` atau message error. |
...
| Admin get health report | `src/services/api/admin.ts` | GET | `/admin/businesses/{publicId}/health-report` | Path `publicId` | `AIReport` | `AIReportPage` (mode admin) menampilkan state yang sama seperti mode user; `404` = belum eligible. |
| Admin regenerate health report | `src/services/api/admin.ts` | POST | `/admin/businesses/{publicId}/health-report/regenerate` | Tidak ada | `AIReport` | `AIReportPage` (mode admin) memanggil ini dari tombol "Generate Ulang". |
