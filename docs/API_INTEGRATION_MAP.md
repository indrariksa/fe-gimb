...
| Get AI report | `src/services/api/businesses.ts` | GET | `/businesses/{publicId}/ai-report` | Path `publicId` | `AIReport` (`status`, `model`, `report`, `error_message`) | `AIReportPage` menampilkan state `processing`/`ready`/`failed`; `404` diperlakukan sebagai "belum eligible" (omzet di bawah threshold), bukan error. |
| Regenerate AI report | `src/services/api/businesses.ts` | POST | `/businesses/{publicId}/ai-report/regenerate` | Tidak ada | `AIReport` | `AIReportPage` memanggil ini dari tombol "Generate Ulang" saat status `failed`; backend menolak `409` di luar kondisi `failed`/`processing` macet. |
...
| Admin get AI report | `src/services/api/admin.ts` | GET | `/admin/businesses/{publicId}/ai-report` | Path `publicId` | `AIReport` | `AIReportPage` (mode admin) menampilkan state yang sama seperti mode user; `404` = belum eligible. |
| Admin regenerate AI report | `src/services/api/admin.ts` | POST | `/admin/businesses/{publicId}/ai-report/regenerate` | Tidak ada | `AIReport` | `AIReportPage` (mode admin) memanggil ini dari tombol "Generate Ulang". |
