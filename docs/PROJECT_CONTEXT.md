...
1. Login memanggil API auth; register email/password membuat akun lalu redirect ke `/registration-success` sambil membawa cooldown resend dari response backend. Jika register sebelumnya timeout setelah user tersimpan tetapi email belum terkirim, retry register dengan email/password yang sama dapat mengembalikan response sukses dan mengirim ulang email verifikasi.
