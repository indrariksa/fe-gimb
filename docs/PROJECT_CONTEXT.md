...
- Laporan Kesehatan Bisnis: halaman `/businesses/:businessId/health-report` (dan versi admin) dengan polling status `processing`/`ready`/`failed`, narasi per sub-skor beserta `score_drivers` dan `alternative_solutions`, kartu "Kelayakan Investasi" (BEP/CAPEX/payback period/ROI dari `report.financial_analysis`, angka dihitung backend bukan Claude, narasi dari Claude — field optional untuk laporan lama sebelum fitur ini ada), 10 chart Chart.js (bar horizontal/radar/doughnut/doughnut-gauge), export PDF/XLSX, dan tombol generate ulang saat gagal; menu sidebar user "Laporan Kesehatan Bisnis" mengikuti pola enable/disable nav lain berdasarkan hasil inventory.
